...
// Esta función es necesaria para el despliegue estático (output: export)
export async function generateStaticParams() {
  return []; // Retornamos vacío ya que los torneos se cargan dinámicamente en el cliente
}
...
export default function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const found = tournaments.find(t => t.id === resolvedParams.id);
    if (found) setTournament(found);
  }, [tournaments, resolvedParams.id]);

  if (!tournament) return (
    <div className="p-20 text-center space-y-4">
      <p className="font-black uppercase text-muted-foreground">Cargando Competición...</p>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-32">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button variant="outline" onClick={() => generateSchedule(tournament.id)} className="rounded-xl font-black h-12">
                ...
                <div className="space-y-4">
                  {tournament.matches.length === 0 ? (
                    <div className="text-center py-20 bg-muted/10 rounded-[2rem] border-2 border-dashed">
                      <p className="font-bold text-muted-foreground uppercase">Sin partidos programados.</p>
                    </div>
                  ) : (
                      ...
                      return (
                        <TableRow key={row.id}>
                          <TableCell className="font-black flex items-center gap-3">
                            ...
                            <span className="truncate max-w-[100px] md:max-w-none">{team?.name}</span>
                          </TableCell>
