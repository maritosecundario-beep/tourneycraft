...
import { Trophy, RefreshCw, ArrowLeft, Star, Coins, Settings2, Trash2, ChevronRight, UserCircle2, Users, AlertTriangle, Plus, X, Sword, Target, CheckCircle2, LayoutGrid, Info, ShieldAlert, TrendingUp, History, ArrowLeftRight } from 'lucide-react';
...
export function TournamentDetailView({ id }: TournamentDetailViewProps) {
  const { tournaments, teams, players, resolveMatch, generateSchedule, resetSchedule, updateTournament, settings, transferPlayer, applySanction, processIncidentDecision } = useTournamentStore();
  ...
  const generateCalculatedScore = (t: Tournament, hTeam?: Team, aTeam?: Team) => {
    ...
    if (t.scoringRuleType === 'bestOfN') {
      hScore = Math.round(val * homeWinProb);
      aScore = val - hScore;
  ...
  const dualStandings = useMemo(() => {
    ...
    return getStandings(tournament.dualLeagueMatches || [], tournament.participants);
  }, [tournament, teams]);
    ...
    <Table>
      ...
      <TableBody>
        {rows.map((row: any, idx: number) => {
          ...
          return (
            <TableRow key={`stand-${row.id}-${idx}`} className={cn(
              isPlayoff && "bg-emerald-500/5",
          ...
          <div key={`mday-${isDual ? 'd' : 'm'}-${day}`} className="space-y-4">
            ...
            <div className="grid gap-3">
              {dayMatches.map((m) => {
                ...
                return (
                  <Card key={`m-row-${m.id}`} onClick={() => m.isSimulated && setSelectedMatchDetail(m)} className={cn("border-none shadow-md transition-all rounded-2xl overflow-hidden cursor-pointer", m.isSimulated && "hover:border-primary/30 border-2 border-transparent")}>
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-32">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" onClick={() => setIsTransferCenterOpen(true)} className="rounded-xl font-black h-12 border-primary text-primary"><Users className="w-4 h-4 mr-2" /> TRASPASOS</Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <Tabs defaultValue="matches">
            ...
            <TabsContent value="matches" className="mt-6 space-y-8">
              {tournament.matches.length === 0 ? <div className="text-center py-20 bg-muted/10 rounded-[2rem] border-2 border-dashed"><p className="font-bold text-muted-foreground uppercase text-xs">Sin partidos programados.</p><Button onClick={() => generateSchedule(tournament.id)} className="mt-4 font-black rounded-xl">GENERAR CALENDARIO</Button></div> : (
                (tournament.leagueType === 'groups' || tournament.leagueType === 'conferences') && tournament.groups ? (
                  <Tabs defaultValue={tournament.groups[0]?.id}>
                    <TabsList className="bg-muted/10 p-1 mb-6 flex overflow-x-auto scrollbar-hide w-full gap-1">
                      {tournament.groups.map((g) => <TabsTrigger key={`cal-tab-${g.id}`} value={g.id} className="text-[10px] font-black uppercase flex-1">{g.name}</TabsTrigger>)}
                    </TabsList>
                    {tournament.groups.map((g) => (
                      <TabsContent key={`cal-cont-${g.id}`} value={g.id} className="space-y-8">
            ...
            <TabsContent value="dual" className="mt-6 space-y-12">
                  ...
                  {tournament.groups ? (
                    <div className="space-y-12 p-6">
                      {tournament.groups.map(group => (
                        <div key={`d-st-${group.id}`} className="space-y-4">
              ...
              <div className="space-y-6">
                <h3 className="text-xl font-black uppercase border-l-4 border-accent pl-4">Calendario Dual (Invertido)</h3>
                {tournament.groups ? (
                  <Tabs defaultValue={tournament.groups[0]?.id}>
                    <TabsList className="bg-muted/10 p-1 mb-6 flex overflow-x-auto scrollbar-hide w-full gap-1">
                      {tournament.groups.map(g => <TabsTrigger key={`d-cal-tab-${g.id}`} value={g.id} className="text-[10px] font-black uppercase flex-1">{g.name}</TabsTrigger>)}
                    </TabsList>
                    {tournament.groups.map(g => (
                      <TabsContent key={`d-cal-cont-${g.id}`} value={g.id} className="space-y-8">
            ...
            <TabsContent value="standings" className="mt-6">
              <Card className="border-none shadow-2xl rounded-[2.5rem] overflow-hidden">
                {tournament.groups ? (
                  <div className="space-y-12 p-6">
                    {tournament.groups.map(group => (
                      <div key={`st-${group.id}`} className="space-y-4">
                        ...
                        {renderStandingsTable(standings.filter(s => group.participantIds.includes(s.id)))}
                      </div>
                    ))}
                  </div>
            ...
            <TabsContent value="news" className="mt-6 space-y-4">
              {tournament.incidents?.length === 0 ? <div className="text-center py-20 bg-muted/10 rounded-[2rem] border-2 border-dashed font-bold text-muted-foreground uppercase text-xs">Sin noticias destacadas.</div> : (
                [...(tournament.incidents || [])].reverse().map((inc) => (
                  <Card key={`news-${inc.id}`} className="border-none shadow-sm rounded-2xl overflow-hidden">
                    <CardContent className="p-4 flex items-center gap-4">
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0", inc.type === 'transfer' ? "bg-accent/10 text-accent" : inc.type === 'sanction' ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary")}>
                        {inc.type === 'transfer' ? <ArrowLeftRight className="w-5 h-5" /> : inc.type === 'sanction' ? <ShieldAlert className="w-5 h-5" /> : <Info className="w-5 h-5" />}
                      </div>
                      <div className="flex-1">
                        ...
                        {inc.status === 'pending' && (
                          <div className="flex gap-2 mt-3">
                            <Button size="sm" className="bg-accent text-white font-black text-[10px]" onClick={() => processIncidentDecision(tournament.id, inc.id, true)}>ACEPTAR FONDOS</Button>
        ...
        <div className="space-y-8">
          <Card className="border-none shadow-xl rounded-[2rem] bg-gradient-to-br from-primary to-primary/80 text-white overflow-hidden">
            <CardHeader className="p-6 pb-0"><CardTitle className="text-2xl font-black uppercase text-white">Leyes del Universo</CardTitle></CardHeader>
          ...
          <Card className="border-none shadow-xl rounded-[2rem]">
            ...
            <CardContent className="space-y-4">
              {standings.slice(0, 3).map((row: any, idx: number) => {
                const team = teams.find(t => t.id === row.id);
                return (
                  <div key={`pod-${row.id}-${idx}`} className="flex items-center gap-4 p-3 bg-muted/20 rounded-xl">
            ...
            return (
              <div className="flex flex-col">
                <div className="bg-muted/10 p-8 border-b text-center space-y-6">
                  <DialogHeader>
                    <DialogTitle className="text-xl font-black uppercase text-center">Informe de Encuentro</DialogTitle>
                    <DialogDescription className="text-center font-bold text-[10px] uppercase text-muted-foreground">Acta oficial de la competición</DialogDescription>
                  </DialogHeader>
                  <div className="flex items-center justify-center gap-8">
                    <div className="flex flex-col items-center gap-2"><CrestIcon shape={home?.emblemShape || 'shield'} pattern={home?.emblemPattern || 'none'} c1={home?.crestPrimary || '#000'} c2={home?.crestSecondary || '#fff'} c3={home?.crestTertiary || '#000'} size="w-16 h-16" /><span className="font-black text-xs uppercase">{home?.name}</span></div>
                    <div className="text-5xl font-black">{selectedMatchDetail.homeScore} - {selectedMatchDetail.awayScore}</div>
                    <div className="flex flex-col items-center gap-2"><CrestIcon shape={away?.emblemShape || 'shield'} pattern={away?.emblemPattern || 'none'} c1={away?.crestPrimary || '#000'} c2={away?.crestSecondary || '#fff'} c3={away?.crestTertiary || '#000'} size="w-16 h-16" /><span className="font-black text-xs uppercase">{away?.name}</span></div>
                  </div>
                </div>
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 bg-card">
                  {[hPlayer, aPlayer].map((p, pIdx) => (
                    <div key={`p-stat-${p?.id || pIdx}`} className="space-y-4">
                      <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center font-black text-primary">#{p?.jerseyNumber || '??'}</div><div><p className="font-black uppercase text-sm">{p?.name || 'Agente Reservado'}</p><p className="text-[10px] font-bold text-accent uppercase">{p?.position || 'N/A'}</p></div></div>
                      {p?.description && <p className="text-[10px] text-muted-foreground italic bg-muted/20 p-3 rounded-xl">{p.description}</p>}
                      <div className="grid grid-cols-2 gap-2">{p?.attributes.map((attr) => (<div key={`attr-${attr.name}`} className="bg-muted/30 p-2 rounded-lg border flex justify-between items-center"><span className="text-[8px] font-black uppercase opacity-50">{attr.name}</span><span className="text-[10px] font-black">{attr.value}</span></div>))}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <Dialog open={!!pendingMatch} onOpenChange={(o) => !o && setPendingMatch(null)}>
        <DialogContent className="max-w-5xl p-0 overflow-hidden rounded-[2.5rem] border-none shadow-2xl bg-card">
          {pendingMatch && (() => {
            ...
            const opponentTeam = teams.find(t => t.id === (isHome ? pendingMatch.match.awayId : pendingMatch.match.homeId));
            const aiPlayer = pendingMatch.aiPlayer;
            return (
              <div className="flex flex-col h-full max-h-[95vh]">
                <div className="bg-primary p-6 md:p-10 text-white border-b-4 border-black/10">
                  <DialogHeader><DialogTitle className="text-2xl md:text-4xl font-black uppercase tracking-tighter text-white">{isHome ? "Local en l'Horta" : "Visitante de l'Horta"}</DialogTitle><DialogDescription className="text-white/70 font-bold uppercase text-[10px] mt-2">Jornada {pendingMatch.match.matchday} Arcade</DialogDescription></DialogHeader>
                </div>
                <div className="flex-1 overflow-y-auto p-4 md:p-10 space-y-8 scrollbar-hide">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div className="p-6 bg-muted/30 rounded-[3rem] space-y-8 border-2 border-dashed border-primary/20">
                      <div className="flex flex-col items-center gap-4 text-center">
                        ...
                        <h3 className="text-2xl font-black uppercase tracking-tighter">{opponentTeam?.name}</h3>
                      </div>
                      {aiPlayer && (
                        <div className="bg-card p-6 rounded-[2rem] shadow-2xl">
                          <div className="flex items-center gap-5 mb-4"><div className="w-12 h-12 rounded-2xl bg-primary text-white flex items-center justify-center font-black text-xl">#{aiPlayer.jerseyNumber}</div><div><p className="font-black uppercase text-xl truncate">{aiPlayer.name}</p><Badge variant="outline" className="text-[10px] font-black">{aiPlayer.position}</Badge></div></div>
                          {aiPlayer.description && <p className="text-[10px] text-muted-foreground italic mb-6 bg-muted/20 p-3 rounded-xl">{aiPlayer.description}</p>}
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{aiPlayer.attributes.map((attr) => (<div key={`opp-a-${attr.name}`} className="bg-muted/40 p-2 rounded-xl border flex flex-col items-center gap-1"><span className="text-[8px] font-black uppercase text-muted-foreground">{attr.name}</span><span className="text-sm font-black text-primary">{attr.value}</span></div>))}</div>
                        </div>
                      )}
                    </div>
                    <div className="flex flex-col gap-8 justify-center bg-muted/10 p-6 rounded-[3rem] border">
                      <div className="space-y-4">
                        <Label className="text-[10px] font-black uppercase text-muted-foreground ml-2">1. Designa tu Agente Élite</Label>
                        <Select value={selectedPlayerId} onValueChange={setSelectedPlayerId}>
                          ...
                          <SelectContent className="rounded-[2rem]">{players.filter(p => p.teamId === tournament.managedParticipantId).map(p => (<SelectItem key={`arcade-p-${p.id}`} value={p.id} className="font-bold uppercase text-xs">#{p.jerseyNumber} {p.name}</SelectItem>))}</SelectContent>
                        </Select>
                      </div>
                      <div className="flex items-center gap-4 justify-center">
                        <Input type="number" value={arcadeHomeScore} onChange={e => setArcadeHomeScore(parseInt(e.target.value) || 0)} className="h-20 w-24 text-4xl font-black text-center rounded-[2rem] border-4" />
                        <span className="text-4xl font-black opacity-10">VS</span>
                        <Input type="number" value={arcadeAwayScore} onChange={e => setArcadeAwayScore(parseInt(e.target.value) || 0)} className="h-20 w-24 text-4xl font-black text-center rounded-[2rem] border-4" />
                      </div>
                    </div>
                  </div>
                </div>
                <div className="p-6 bg-muted/20 border-t flex flex-col md:flex-row gap-4"><Button variant="ghost" onClick={() => setPendingMatch(null)} className="flex-1 h-16 font-black rounded-3xl">CANCELAR</Button><Button disabled={!selectedPlayerId} onClick={executeArcadeDuel} className="flex-[2] h-16 font-black rounded-3xl bg-primary text-white">GRABAR RESULTADO</Button></div>
              </div>
            );
          })()}
        </DialogContent>
        ...
        <DialogContent className="max-w-2xl rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden">
          ...
          <div className="p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2"><Label>Seleccionar Jugador</Label><Select onValueChange={setSelectedPlayerId}><SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Elegir..." /></SelectTrigger><SelectContent>{players.map(p => (<SelectItem key={`tr-p-${p.id}`} value={p.id}>{p.name} ({teams.find(t => t.id === p.teamId)?.name || 'Libre'})</SelectItem>))}</SelectContent></Select></div>
              <div className="space-y-2"><Label>Destino</Label><Select onValueChange={(v) => { if (selectedPlayerId) { transferPlayer(selectedPlayerId, v === 'free' ? undefined : v); toast({ title: "Traspaso Ejecutado" }); setIsTransferCenterOpen(false); } }}><SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Destino..." /></SelectTrigger><SelectContent><SelectItem value="free">Agente Libre</SelectItem>{teams.map(t => (<SelectItem key={`tr-t-${t.id}`} value={t.id}>{t.name}</SelectItem>))}</SelectContent></Select></div>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isSanctionMenuOpen} onOpenChange={setIsSanctionMenuOpen}>
        <DialogContent className="max-w-2xl rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden">
          <div className="bg-destructive p-6 text-white border-b">
            ...
            <DialogDescription className="text-white/80 font-bold text-xs uppercase mt-1">Aplica sanciones económicas o suspensiones.</DialogDescription>
          </div>
          <Tabs defaultValue="team">
            <TabsList className="grid grid-cols-2 rounded-none h-14 bg-muted/20 border-b"><TabsTrigger value="team" className="font-black text-xs uppercase">Sancionar Club</TabsTrigger><TabsTrigger value="player" className="font-black text-xs uppercase">Suspender Jugador</TabsTrigger></TabsList>
            <TabsContent value="team" className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2"><Label>Seleccionar Club</Label><Select onValueChange={setSelectedPlayerId}><SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Elegir..." /></SelectTrigger><SelectContent>{teams.map(t => (<SelectItem key={`sanc-t-${t.id}`} value={t.id}>{t.name}</SelectItem>))}</SelectContent></Select></div>
                <div className="space-y-2"><Label>Multa ({settings.currency})</Label><Input type="number" placeholder="Cantidad..." onKeyDown={(e) => { if (e.key === 'Enter' && selectedPlayerId) { applySanction(tournament.id, 'team', selectedPlayerId, Number((e.target as HTMLInputElement).value)); setIsSanctionMenuOpen(false); toast({ title: "Sanción Aplicada" }); } }} className="h-12 rounded-xl" /></div>
              </div>
            </TabsContent>
            <TabsContent value="player" className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2"><Label>Seleccionar Jugador</Label><Select onValueChange={setSelectedPlayerId}><SelectTrigger className="h-12 rounded-xl"><SelectValue placeholder="Elegir..." /></SelectTrigger><SelectContent>{players.map(p => (<SelectItem key={`sanc-p-${p.id}`} value={p.id}>{p.name}</SelectItem>))}</SelectContent></Select></div>
                <div className="space-y-2"><Label>Jornadas de Suspensión</Label><Input type="number" placeholder="Jornadas..." onKeyDown={(e) => { if (e.key === 'Enter' && selectedPlayerId) { applySanction(tournament.id, 'player', selectedPlayerId, Number((e.target as HTMLInputElement).value)); setIsSanctionMenuOpen(false); toast({ title: "Jugador Suspendido" }); } }} className="h-12 rounded-xl" /></div>
              </div>
            </TabsContent>
          </Tabs>
        </DialogContent>
          ...
          <div className="space-y-8 py-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-2 md:col-span-2"><Label>Sistema de Marcador</Label><Select value={editScoringType} onValueChange={(v: any) => setEditScoringType(v)}><SelectTrigger className="rounded-xl h-12"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="bestOfN">Sets (Suma Exacta)</SelectItem><SelectItem value="firstToN">Primero en marcar N</SelectItem><SelectItem value="nToNRange">Rango puntuación</SelectItem></SelectContent></Select></div>
                <div className="space-y-2"><Label>Valor Meta</Label><Input type="number" value={editScoringValue} onChange={e => setEditScoringValue(Number(e.target.value))} className="rounded-xl h-12" /></div>
              </div>
            </div>
            <div className="space-y-4">
              <h4 className="text-[10px] font-black uppercase border-b pb-2 text-primary">Estructura de Tabla</h4>
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1"><Label className="text-[9px] uppercase font-bold text-center block">V.</Label><Input type="number" value={editWinPts} onChange={e => setEditWinPts(Number(e.target.value))} className="text-center h-10" /></div>
                <div className="space-y-1"><Label className="text-[9px] uppercase font-bold text-center block">E.</Label><Input type="number" value={editDrawPts} onChange={e => setEditDrawPts(Number(e.target.value))} className="text-center h-10" /></div>
                <div className="space-y-1"><Label className="text-[9px] uppercase font-bold text-center block">D.</Label><Input type="number" value={editLossPts} onChange={e => setEditLossPts(Number(e.target.value))} className="text-center h-10" /></div>
              </div>
            </div>
          </div>
          <DialogFooter className="gap-2"><Button variant="ghost" onClick={() => setIsEditing(false)} className="rounded-xl font-black">CANCELAR</Button><Button onClick={() => { if (!tournament) return; updateTournament({ ...tournament, name: editName, sport: editSport, winPoints: editWinPts, drawPoints: editDrawPts, lossPoints: editLossPts, scoringRuleType: editScoringType, scoringValue: editScoringValue, playoffSpots: editPlayoffSpots, relegationSpots: editRelegationSpots, variability: editVariability }); setIsEditing(false); toast({ title: "Ajustes Guardados" }); }} className="font-black rounded-xl px-8 shadow-lg shadow-primary/20 bg-primary">GUARDAR TODO</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
